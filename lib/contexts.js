...
// Find the closest (most relevant) path
Context.prototype.getClosestPath = function() {
  var context = this;
  while (context) {
    if (context && context.expression && context.expression.isPathExpression()) return context.expression.pathSegments(context).join('.');
    context = context.parent;
  }
};
